...
# Revision History
#
# R. Hempel 26Dec03 - puts to the console are commented out
#                   - Allow fast firmware download again
...
wm title . "pbForth Console 2.2.0"
...
proc ::rcxtk::uploadFirmware { speed } {
  ...
  if { [regexp -- "fast" $speed] } {
    switch -regexp -- $::rcxOption::towerPort {
      {com*}          { catch {::rcxfirm::uploadImage $::rcxfirm::fastSerialLoad "slow"} } \
      {.*LEGOTOWER.*} { catch {::rcxfirm::uploadImage $::rcxfirm::fastUSBLoad    "slow"} } \
    }
  }

  ::rcxutil::showLog "Preload Image loaded..."
  
  catch {::rcxfirm::uploadImage [read $fileID] $speed}

  ::rcxutil::showLog "Image loaded..."
...
proc ::rcxtk::Console { } {
  ...
  while {1} {
    ...
    puts "$rcxterm::rcvEvent $::rcxterm::rcvTimeout"

    set s [::rcxterm::readConsole]

    if { $::rcxterm::rcvTimeout } {
      continue
    }
