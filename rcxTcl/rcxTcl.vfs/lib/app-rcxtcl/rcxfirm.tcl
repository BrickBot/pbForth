...
# R. Hempel 26Dec03 - add fastSerialLoad and fastUSBLoad
#                   - rcvHandler now required 6 timeouts before giving up
#                   - puts to the console are commented out
#------------------------------------------------------------------------------
...
set ::rcxfirm::fastSerialLoad "\
S11880007906000F6B86EE807906EE646DF67906EE745E003BCC\n\
S11880159A0B877906EE5E5E000688FE673ED97FD870507FD885\n\
S118802A704018EE6A8EEF51FE026A8EEF06FE0D6A8EEE5E54BF\n\
S118803F70446F20796F7520627974652C207768656E2049202D\n\
S10980546B6E6F636B3FCD\n\
S90380007C"

set ::rcxfirm::fastUSBLoad "\
S11880007906000F6B86EE807906EE646DF67906EE745E003BCC\n\
S11880159A0B877906EE5E5E0006887FD870507FD8704018EE4B\n\
...
proc ::rcxfirm::rcvHandler { } {
  ...
  #---------------------------------------------------------------------------
  # Reset the rcv character timer to 50 milliseconds
  ...
  if { 0 == [string length $inData] } {
    if { 6 == [incr rcxfirm::rcvHandlerCount] } {
...
# TEST
# binary scan $msg H* outString
#
# puts "$rcxfirm::rcvEvent -> [string length $msg] -> $outString"
# END TEST

# puts "Expecting $rlen bytes of data"
  
  set ::rcxfirm::rcvHandlerAID [after 50 {::rcxfirm::rcvHandler}]

  rcxComm::portWrite $::rcxComm::portID $msg
  
  while { 0 == [regexp {TIMEOUT|RXDONE|EOF|RXOVERRUN} $rcxfirm::rcvEvent] } {
    vwait rcxfirm::rcvEvent
#   puts "$rcxfirm::rcvEvent"
  }
...
proc rcxfirm::uploadImage { image speed } {
   ...
   if { 0 != [regexp -- "fast" $speed] } {
     ...
     ::rcxComm::portOpen $::rcxOption::towerPort 4800 o
   } else {
